...
INITIAL_POD_COUNT=$(kubectl get pods -n ${NAMESPACE} -l app.kubernetes.io/name=learn-node --field-selector=status.phase=Running --no-headers | wc -l)
...
sleep 5
kubectl wait --for=condition=Ready pod -l app.kubernetes.io/name=learn-node -n ${NAMESPACE} --timeout=120s

FINAL_POD_COUNT=$(kubectl get pods -n ${NAMESPACE} -l app.kubernetes.io/name=learn-node --field-selector=status.phase=Running --no-headers | wc -l)
